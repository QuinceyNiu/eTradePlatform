...
import com.comz.store.service.ex.UpdateException;
...
public class AddressController extends BaseController{
    ...
    public JsonResult<Void> update(Address address, HttpSession session) {
        ...
        Integer result = addressService.updateAddress(address, modifiedUser);
        if (result == 0) {
            throw new UpdateException("Unknown exception when modifying address");
        }
        return new JsonResult<>(OK);
    }
